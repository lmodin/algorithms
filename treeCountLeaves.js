...
Tree.prototype.countLeaves = function () {
  ...
  var leafCount = 0;
  //create a recursor to search the tree
  //should take parameter: tree
  var checkForLeaves = function(tree) {
    //check if tree.children.length > 0
    if (tree.children.length > 0) {
      //if yes, iterate through the children and call recursor on each one
      for (var i = 0; i < tree.children.length; i++) {
        checkForLeaves(tree.children[i]);
      }
    } else {
      //otherwise
      //add to counter, return
      leafCount++;
      return;
    }
  }
  //call recursor on this;
  checkForLeaves(this);
  //return counter
  return leafCount;
};
