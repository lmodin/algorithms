...
var mergeSort = function(array) {
  ...
  //create a first half variable
  var firstHalf = array.splice(0, Math.floor(array.length / 2));
  //create a second half variable
  var secondHalf = array;

  //define a variable as the result of calling mergesort on the first half
  var firstHalfSorted = mergeSort(firstHalf);
  //another variable as the result of calling mergesort on the second half
  var secondHalfSorted = mergeSort(secondHalf);
  //call the sorting helper and return the resulting array
  return sorter(firstHalfSorted, secondHalfSorted);
};

var sorter = function(first, second) {
  //create a result array
  var result = [];
  //while the length of both is is greater than zero,
  while (first.length > 0 && second.length > 0) {
    //compare the values at 0 of each array
    if (first[0] <= second[0]) {
      //break off the smaller and push it to result
      result.push(first[0])
      first = first.slice(1)
    } else {
      //break off the smaller and push it to result
