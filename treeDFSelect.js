...
Tree.prototype.DFSelect = function(filter) {
  //create an array to which we'll push our values
  var filteredChildren = [];
  var recursor = function (node, depth) {
    //console.log()
    if (filter(node.value, depth)) {
      filteredChildren.push(node.value);
    }
    if (node.children) {
      for (var i = 0; i < node.children.length; i++) {
        recursor(node.children[i], depth + 1);
      }
    } else {
      return;
    }
  }
  //call recursor on tree, depth
  recursor(this, 0);
